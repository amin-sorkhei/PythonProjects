...
def file_reader(input_address):
    ...
    print 'all files in the directory' + ' '.join(all_files)
    for item in all_files:
        absolute_address = os.path.join(input_address, item)  # creates the valid address of the xml files
        if absolute_address[-3:] == 'xml':  # in order only to process xml files
            print 'the chosen file: ' + absolute_address
            current_file = open(absolute_address, 'rU')
            raw_text = current_file.read()
            print 'Extracting abstracts initiated'
...
# ---------------------------------------------------------------------------------------------
def main():
    address = '/home/sorkhei/PycharmProjects/8000_abstracts'
    file_reader(address)


    remove_latex('1-ALL_ABSTRACTS.txt')
    remove_stop_words('2-NoFormulaAbstract.txt')
    stem_word_dic = find_stems('3-Non_stop.txt')
    least_frequent_words_finder('4-Stems.txt', 4)
    doc_word_frequency('4-Stems.txt')
    # ---------------------------this part may change due to nature of different projects
    # ---------------------------Note that redundant_stem.txt has been created manually Based on suggested words Dorota
    with open('redundant_stems.txt') as redundant_words_file:
        redundant_words_stems = [wrd.strip() for wrd in redundant_words_file.readlines()]
        print redundant_words_stems
        redundant_words = []
        for word in redundant_words_stems:
            redundant_words += stem_word_dic[word]
        redundant_words.remove('generative')  # Dorota asked for this to exclude this word from stop words
        f = open('extenden_stopwords', 'w')
        f.write(' '.join(redundant_words))
        f.close()
...
'''print raw_frequency_dic
    ...
    with open('raw_count_over_2000.txt', 'w') as outfile:
        outfile.write('stems \t count \t original words \n')
        for wrd, count in sorted_raw_count_list:
            if count > 1999:
                outfile.write(wrd + ' \t ' + str(count) + ' \t ' + '--'.join(stem_word_dictionary[wrd]) + '\n')
            else:
                break'''
