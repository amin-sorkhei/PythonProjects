        ...
        for w in line.split():
            ...
            if match:
                if len(match.group(1)) > 3:
                    words.append(match.group(1))  # if the words passes the filter it appends to tmp
        for w in words:
            stem = porter.stem(w)
...
# ---------------------------------------------------------------------------------------------
# This is an INDEPENDENT function to remove the given list of words from a file
# input: the input file name and list of the words
# output: a file called 'words_removed.txt'
# Note, you are the one who needs to take care of the PUNCTUATIONS in the input file


def remove_words(input_file_name, list_of_words):
    f_in = open(input_file_name, 'rU')
    f_out = open('words_removed', 'w')
    lines = f_in.readlines()
    for line in lines:
        words = line.split()
        words = [w for w in words if w not in list_of_words]
        result = ' '.join(words)
